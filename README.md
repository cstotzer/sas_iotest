# sas_iotest.sh - A utility to accurately measure I/O performance on unix/linux.
...
`sas_iotest.sh` aims to provide a lightweight, flexible alternative that:
...
Running `sas_iotest.sh` requires `fio` - the Flexible I/O Tester which can be installed on most Linux flavours using the standard package manager as well as on Mac OS (using `homebrew`). As `fio` can also be installed on Windows it might also be possible to run `sas_iotest.sh` on Windows Systems.
...
`sas_iotest.sh` comes with the follwing files: 
...
* `sas_iotest.sh` The Projects main excutable
...
Before running `sas_iotest.sh` it needs to be configured. You will find a sample configuration in sample.env. 
...
While the configuration step might seem daunting at first, runninig the utility is done by simply invoking the script by typing `./sas_iotest.sh` in `IOTEST_DIR`. Configuration ensures that a test can be repeated any number of times using the same parameters whitout juggling with complex parameter lists on the command line.
...
After `sas_iotest.sh` has been run, running `python3 collect_fio_json_to_csv.py` will produce a summary table in the file `fio_summary.csv`. This file can be imported to excel for analysis or graphical representation.
